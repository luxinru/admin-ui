    ...
    <el-dialog :title="title" v-model="open" width="50vw" top="5vh" append-to-body>
      <div class="app-container">
        ...
        <el-table  @current-change="getRowData"  highlight-current-row v-loading="loading" :data="dataList" @row-dblclick="toDownLevel" >
          <el-table-column label="单位编码" prop="departCode" />
...
import {listDictDeptExtend} from "@/api/system/deptExtend";
import { Base64 } from "js-base64";
import {ref} from "@vue/runtime-core";
const emit = defineEmits(['handleDepartCard'])
...
function getList() {
  loading.value = true;
  listDictDeptExtend(proxy.addDateRange(queryParams.value)).then(res => {
    loading.value = false;
...
function getPageList() {
  ...
  if (dataList.value.length > 0){
    queryParams.value.dataFilter = Base64.encode(" and parent_code = '"+dataList.value[0].parentCode+"' ");
    listDictDeptExtend(proxy.addDateRange(queryParams.value)).then(res => {
      loading.value = false;
      dataList.value = res.rows;
      total.value = res.total;
...
/** 查看打开方法 */
function openCard(titleParam) {
  clearQueryParam();
  title.value = titleParam;
  open.value = true;
  queryParams.value.departLevel = "1";
  getList();
}
...
function toDownLevel(row){
  ...
  if (rowData.markTail == '1'){
    // 明细数据,传回父组件
    emit('handleDepartCard',rowData);
    close();
  } else {
    clearQueryParam();
    // 非明细数据，查询下级
    queryParams.value.dataFilter = Base64.encode(" and parent_code = '"+rowData.departCode+"' ");
    getList();
    // 存map,给上级按钮用
    map.value.set(rowData.departLevel,dataList.value);
    mapTotal.value.set(rowData.departLevel,total.value);
  }
}
...
function backUpLevel(){
  ...
  clearQueryParam();
  if (dataList.value.length > 0){
    let currentLevel = dataList.value[0].departLevel;
    // 判断是否最上级
    if (currentLevel == "1"){
...
function clearQueryParam() {
  queryParams.value.departCode = null;
  queryParams.value.departName = null;
  queryParams.value.departLevel = null;
