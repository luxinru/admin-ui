    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" :xs="24">
        <div class="head-container">
          <el-input v-model="contentsName" @input="onQueryChanged" placeholder="请输入目录名称" clearable prefix-icon="Search"
            style="margin-bottom: 20px" />
        </div>
        <div class="head-container">
          <!-- <el-tree :data="assetOptions" :props="defaultProps" :expand-on-click-node="false"
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryParams" :inline="true" v-show="showSearch" label-width="68px">
          ...
          <el-form-item label="定义级别" prop="dataLevel">
        ...
        <el-table v-loading="loading" :data="assetList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="目录编号" align="center" key="contentsCode" prop="contentsCode"
            v-if="columns[0].visible" />
          <el-table-column label="目录名称" align="center" key="contentsName" prop="contentsName" v-if="columns[1].visible"
            :show-overflow-tooltip="true" />
          <el-table-column label="目录级别" align="center" key="codeLevel" prop="codeLevel" v-if="columns[3].visible"
            width="120" />
          <el-table-column label="父编码" align="center" key="parentContentsCode" prop="parentContentsCode"
            v-if="columns[2].visible" :show-overflow-tooltip="true" />
          <el-table-column label="单位编码" align="center" key="departCode" prop="departCode" v-if="columns[4].visible"
            :show-overflow-tooltip="true" />
          <el-table-column label="单位名称" align="center" key="deptName" prop="deptName" v-if="columns[5].visible"
            :show-overflow-tooltip="true" />
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="目录编码" prop="contentsCode">
              <el-input v-model="form.contentsCode" placeholder="请输入目录编码" maxlength="30" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="目录名称" prop="contentsName">
              <el-input v-model="form.contentsName" placeholder="请输入目录名称" maxlength="30" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="目录级别" prop="codeLevel">
              <el-input v-model="form.codeLevel" placeholder="请选择目录级别" maxlength="11" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="父编码" prop="parentContentsCode">
              <el-input v-model="form.parentContentsCode" placeholder="请输入父编码" maxlength="50" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="单位名称" prop="departCode">
              <treeselect v-model="form.departCode" :options="deptExtendTreeList" :normalizer="normalizer"
                :show-count="true" placeholder="请选择单位名称" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="计量单位">
              <el-input v-model="form.unitCode" placeholder="请输入计量单位" maxlength="20" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
export default {
  ...
  data() {
    return {
      ...
      open: false,
      dict: this.useDict("sys_means_type", "sys_data_level", "sys_xn_yes_no"),
      // 列信息
      columns: [
  ...
  methods: {
    ...
    onQueryChanged(contentsName) {
      ...
      if (contentsName == "") {
        ...
        if (contentsName != null) {
          this.$refs.treeRef.filter(contentsName)

        }
      }
    },

    filterMethod(contentsName, node) {
      if (!contentsName) return true;
      // console.log(node.contentsName.indexOf(contentsName));
      return (node.contentsName.indexOf(contentsName) !== -1);
      return true;
    },
    ...
    handleStatusChange(row) {
      ...
      // this.$modal.confirm('确认要"' + text + '""' + row.contentsName + '"吗？').then(function () {
    ...
    filterNode(value, data) {
      ...
      return data.contentsName.indexOf(value) !== -1;
    },
    // 节点单击事件
    handleNodeClick(data) {
      this.queryParams.parentContentsCode = data.contentsCode;
      this.handleQuery();
    },
    ...
    /** 重置按钮操作 */
    resetQuery() {
      this.queryParams.parentContentsCode = null;
      this.getTreeselect();
