<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd"
          v-hasPermi="['system:privateConfig:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate"
          v-hasPermi="['system:privateConfig:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['system:privateConfig:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Download" @click="handleExport"
          v-hasPermi="['system:privateConfig:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="admConfigList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="  区分单位编码" align="center" key="partDepartCode" prop="partDepartCode" />
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="区分单位编码" prop="partDepartCode">
              <el-input v-model="form.partDepartCode" placeholder="请输入区分单位编码" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="单位名称" prop="departName">
              <el-input v-model="form.departName" placeholder="请输入单位名称" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
<script>
import { listAdmConfig, getAdmConfig, delAdmConfig, addAdmConfig, updateAdmConfig } from "@/api/system/admConfig/admconfig";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listAdmConfig(this.queryParams).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateAdmConfig(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAdmConfig(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除参数表编号为"' + ids + '"的数据项？').then(function () {
        return delAdmConfig(ids);
      }).then(() => {
        this.getList();
