<template>
  <div class="div-editor-container-grey">
    <el-row>
          ...
          <el-table v-loading="loading" :data="postList" @selection-change="handleSelectionChange" :height="height">
            ...
            <el-table-column label="单据编号" align="center" prop="invoiceNumber" width="220" :show-overflow-tooltip="true">
              <template v-if="sendButtonShow" #default="scope">
                <router-link :to="{ path:'/repair/apply/detailshow',
                              query:{
              ...
              <template v-else #default="scope">
                <router-link :to="{ path:'/repair/apply/detail',
                              query:{
                                invoiceNumber:scope.row.invoiceNumber
                              }
                        }"  class="link-type">
      ...
      <!-- 附件弹窗 -->
      <attachment-data ref="attachmentForm"/>
...
<script setup>
import { listHouseRefineInvoice, delHouseRefineInvoice, checkSendAuditDetail, sendInvoice, cancelSendData } from "@/api/assets/house/repair/invoice";
import { Base64 } from "js-base64"
import attachmentData from "@/views/assets/house/repair/components/attachment/index";
// 附件弹窗
...
function sendAudit() {
  checkSendAuditDetail(selectInvoiceNumber.value).then(response => {
    if(response.msg != ""){
      ...
      proxy.$modal.confirm('是否确认提交选中的数据项?', "提示", {
...
function cancelSend() {
  ...
  proxy.$modal.confirm('是否确认取回选中的数据项?', "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(function() {
...
function attachmentOpenFun() {
  attachmentForm.value.editOpen("附件弹窗");
}
