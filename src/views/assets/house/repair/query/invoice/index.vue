          ...
          <el-table v-loading="loading" :data="postList" @selection-change="handleSelectionChange" :height="height">
            ...
            <el-table-column label="单据编号" align="center" prop="invoiceNumber" width="220" :show-overflow-tooltip="true">
              <template #default="scope">
                <router-link :to="{ path:'/repair/implementation/detailsearch',
                              query:{
...
<script setup>
import { listHouseRefineInvoice, revocationInvoiceData, goBackData } from "@/api/assets/house/repair/invoice";
...
// 单据状态
const invoiceStatusOptions = ref([{codeProperty1: '01', codeName1: '新建'}, {codeProperty1: '02', codeName1: '被退回'}, {codeProperty1: '03', codeName1: '审批中'}, {codeProperty1: '04', codeName1: '实施退回'}, {codeProperty1: '05', codeName1: '实施中'}, {codeProperty1: '06', codeName1: '实施完成'}]);
//高度
...
function revocation() {
  // 05实施退回撤销单据
  const selectInvoiceNumbers05 = ref([]);
  // 06实施完成撤销单据
  const selectInvoiceNumbers06 = ref([]);

  for (let i = 0; i < selectionData.value.length; i++) {
    const invoiceStatusTemp = selectionData.value[i].invoiceStatus;
    if(invoiceStatusTemp == '06' || invoiceStatusTemp == '05'){
      if(invoiceStatusTemp == '06'){
        selectInvoiceNumbers06.value[i] = selectInvoiceNumber.value[i];
      }else{
        selectInvoiceNumbers05.value[i] =selectInvoiceNumber.value[i];
      }
    }else{
      proxy.$modal.msgSuccess("选择数据中存在未实施完成或实施退回状态，无法撤销");
      return;
    }
  }

  if(selectInvoiceNumbers05.value.length > 0 || selectInvoiceNumbers06.value.length > 0){
    const objRevocationData06 = {
      selectInvoiceNumbers: selectInvoiceNumbers06.value
    };
    const objRevocationData05 = {
      selectInvoiceNumbers: selectInvoiceNumbers05.value,
      invoiceStatus: '04'
    };
    proxy.$modal.confirm('是否确认撤销选中的数据项?', "提示", {
      ...
      if(selectInvoiceNumbers06.value.length > 0){
        revocationInvoiceData(objRevocationData06);
      }
      if(selectInvoiceNumbers05.value.length > 0){
        goBackData(objRevocationData05);
      }
    }).then(() => {
      getList();
...
const data = reactive({
  ...
  queryParams: {
    ...
    departCode: null,
    invoiceDate: null,
    dataFilter: Base64.encode(" and invoice_status in ('01','02','03','04','05','06')")
  }
});
...
function handleDetail() {
  router.push({
    path: "/repair/implementation/detailsearch",
    query: {
