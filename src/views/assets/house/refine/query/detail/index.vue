                ...
                <el-row :gutter="10" class="mb8">
                    <el-col :span="1.5">
                        <el-button
                                ...
                                plain
                                icon="Position"
                                :disabled="cancelHouseRefineAssetsButton"
                                @click="cancelHouseRefineAssets"
                        >撤销</el-button>
                    </el-col>
...
<script setup>
   import { cancelHouseRefineAssetsData, cancelAssetsData,
       addHouseRefineInvoice, getHouseRefineInvoice, updateHouseRefineInvoice,
       getAssetsGetList, delAssetsDetail,
       addHouseRefineDeatil, updateHouseRefineDeatil, delHouseRefineDetail,
       getHouseRefineDetail, getHouseRefineGetList } from "@/api/assets/house/refine/detail";
   ...
   // --------------最下方房屋信息变量定义----------------
   const houseRefineAssetsCodes = ref([]);
   // 撤销房屋细化资产
   const cancelHouseRefineAssetsButton = ref(true);
   // 新增按钮，是否可点击
   ...
   function assetsHandleSelectionChange(selection) {
       ...
       if(selection.length == 0 || selection.length > 1){
           ...
           // 查询房屋卡片列表数据，入参
           houseRefineQueryParams.value.assetsCode = assetsCodes.value[0]==""?"-1":assetsCodes.value[0];
   ...
   function cancelAssets() {
       if(parentInvoiceStatus.value != '04'){
           proxy.$modal.msgSuccess("单据为未审批完成状态，无法撤销明细数据！");
           return;
       }
       const obj = {
           invoiceNumber: parentInvoiceNumber.value,
           assetsCodes: assetsCodes.value
       };
   ...
   function houseRefineHandleSelectionChange(selection) {
       ...
       cancelHouseRefineAssetsButton.value = !selection.length;
       houseRefineAssetsCodes.value = selection.map(item => item.assetsCode);
   }
   // 卡片保存，子页面回调函数
   ...
   function handleAddHouseRefineAssets() {
      debugger;
      cardAssetsForm.value.addOpen(parentInvoiceNumber, "房屋新增卡片", false, false, false, true);
   }
   /** 修改房屋卡片按钮操作 */
   function handleLookHouseRefineAssets(row) {
       if(row.id != ""){
           ...
           getHouseRefineDetail(row.id).then(response => {
               ...
               cardAssetsForm.value.viewOpen(mapFormData, "房屋查看卡片", false, false, false, true);
           });
       }
   }
   /** 修改房屋卡片按钮操作 */
   function handleUpdateHouseRefineAssets(row) {
      ...
      if(id != ""){
          ...
          getHouseRefineDetail(id).then(response => {
              mapFormData.set("houseOpenFormData", response.data);
             cardAssetsForm.value.updateOpen(mapFormData, "房屋修改卡片", false, false, false, true);
         });
      }
   }
   // 删除房屋细化资产
   ...
   // 撤销房屋细化资产
   function cancelHouseRefineAssets() {
       if(parentInvoiceStatus.value != '04'){
           proxy.$modal.msgSuccess("单据为未审批完成状态，无法撤销明细数据！");
           return;
       }
       const obj = {
           invoiceNumber: parentInvoiceNumber.value,
           assetsCodes: houseRefineAssetsCodes.value
       };
       proxy.$modal.confirm('是否确认撤销选中的数据项？').then(function() {
           return cancelHouseRefineAssetsData(obj);
       }).then(() => {
           houseRefineGetList();
           proxy.$modal.msgSuccess("撤销成功");
       }).catch(() => {});
