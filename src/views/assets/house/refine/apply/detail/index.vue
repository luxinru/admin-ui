                        ...
                        <el-row>
                            ...
                            <el-col :span="6">
                                <el-form-item label="单据状态" prop="invoiceStatus">
                                    <el-select v-model="invoiceForm.invoiceStatus" placeholder="请选择" @change="dataSourcesFunction" disabled="false">
                                        <el-option
                            ...
                            <el-col :span="6">
                                <el-form-item label="单据类型" prop="invoiceType">
                                    <el-select v-model="invoiceForm.invoiceType" placeholder="请选择" @change="dataSourcesFunction" disabled="false">
                                        <el-option
                                                v-for="item in invoiceStatusOption"
                                                :key="item.value"
                                                :label="item.label"
...
<!--                        <el-button type="primary" @click="sendAudit">送 审</el-button>-->
                    </div>
                </div>
            </el-card>
                ...
                <el-row :gutter="10" class="mb8">
                    ...
                    <el-col :span="1.5">
                        <el-button
                                ...
                                @click="handleDeleteHouseRefineAssets"
                        >删除</el-button>
                    </el-col>
                    <el-col :span="1.5">
                        <el-button
...
<script setup>
   ...
   function checkAssets() {
       ...
       chooseAssetsRef.value.openSelect(config)
   }
   // 删除资产
   function handleDeleteAssets() {
       const obj = {
           invoiceNumber: parentInvoiceNumber.value,
           mainAssetsKeyCodes: globalMainAssetsKeyCodes.value
       };
   ...
   // 选资产弹窗确认保存事件回调
   function handleAllDetailData(val) {
       handleDetailData(val);
   }
   function handleDetailData(dataList) {
       let message = "";
       // alert("我的"+val[0].assetsCode);
       // alert("我的"+val.length);
       ...
       for (let i = 0; i < dataList.length; i++) {
           ...
           /*if(contentAssetsCode != '16'){
               message+= "资产"+assetsCode+"不存在细化资产信息，不允许进行细化！";
           }*/
       }
   ...
   function saveSubmitForm(arrForm) {
       if(arrForm.get("nextStepSureFunSign")){
           ...
           if(null!=arrForm.get("houseOpenFormData") && arrForm.get("houseOpenFormData").keyCode != undefined){
               ...
               arrForm.get("houseOpenFormData").copyNumber = '0';
   ...
   function handleDeleteHouseRefineAssets(row) {
       ...
       proxy.$modal.confirm('是否确认删除选中的数据项123？').then(function() {
   ...
   function handleCopyData(num) {
       houseRefineRowData[0].pkid = houseRefineQueryParams.value.pkid;
       houseRefineRowData[0].keyCode = keyCodeTemp;
       houseRefineRowData[0].assetsCode = assetsCodeTemp;
