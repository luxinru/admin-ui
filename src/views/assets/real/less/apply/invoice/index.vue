<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="invoiceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="减少类型" key="lessTypeCode" align="center" prop="lessTypeCode" width="150" />
      ...
      <el-table-column label="制单日期" key="invoiceDate" align="center" prop="invoiceDate" />
    ...
    <el-dialog :title="title" v-model="open" width="600px" append-to-body>
      <el-form :model="form" ref="formRef" label-width="80px">
        ...
        <el-form-item label="单据类型"> 减少-实物资产 </el-form-item>
        <el-form-item label="单位名称">
          {{ form.departName }}
        </el-form-item>
        <el-form-item label="减少类型" prop="lessTypeCode">
          <el-select v-model="form.lessTypeCode" style="width: 240px" clearable placeholder="请选择减少类型" >
            <el-option v-for="dict in lessTypeCodeOption" :key="dict.lessTypeCode" :label="dict.lessTypeName"
                       :value="dict.lessTypeName" />
          </el-select>
        </el-form-item>
...
<script setup>
import { listInvoice, addInvoice, updateInvoice, getInvoice, delInvoice, sendInvoice } from "@/api/assets/real/less/invoice";
import { checkDetail } from "@/api/assets/real/add/detail";
...
// 单据状态
const lessTypeCodeOption = ref([{lessTypeCode: '01', lessTypeName: '类型1'}, {lessTypeCode: '02', lessTypeName: '类型2'}]);
// 单据类型
...
function handleAdd() {
  open.value = true;
  title.value = "资产增加单据";
  form.value.assetsType = "0109";
  form.valueinvoiceType = "01";
  form.value.invoiceStatus = "01";
}
...
function handleDelete(row) {
  ...
  let num = row.invoiceNumber || number.value;
  proxy.$modal.confirm('是否确认删除单据编号为"' + num + '"的数据项?', "警告", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(function() {
    return delInvoice(rowIds);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download("business/less/invoice/export", {
    ...queryParams.value
