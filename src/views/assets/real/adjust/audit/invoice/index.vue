...
<script setup>
import {listAdjust,invoiceAudit} from "@/api/assets/real/adjust/invoice";
...
// 单据状态
const invoiceStatusOptions = ref([{codeProperty1: '03', codeName1: '审批中'}, {codeProperty1: '04', codeName1: '审批完成'}]);
// 单据类型
...
queryParams.value.dataFilter = Base64.encode(" and invoice_status in ('03','04')");
// 表单参数
const form=ref({});
getList();
...
function handleAudit(flag, reason) {
  ...
  invoiceAudit(proxy.addDateRange(obj)).then(() => {
    getList();
    if(flag){
