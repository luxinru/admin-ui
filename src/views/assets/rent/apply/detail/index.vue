...
<script setup>
import {listDetail,addDetail,updateDetail,delDetail,getDetail} from "@/api/assets/rent/apply/detail";
...
function handleView() {
  ...
  getDetail(id).then(response => {
    card.value.view(response.data,"资产明细","form");
  });
}
/** 修改按钮操作 */
function handleUpdate(row) {
  var id = row.id || ids.value
  getDetail(id).then(response => {
    card.value.update(response.data,"资产明细","form");
  });
}
/** 提交按钮 */
function submitForm(form) {
  form.value.invoiceNumber = defaultNumber.value;
...
function handleExport() {
  proxy.download("rent/apply/detail/export", {
    ...queryParams.value,
